...
export async function seedDatabaseIfEmpty(force = false) {
  ...
  // 5. Seed Instagram Posts / Videos (Delete default mock ones to let seller manage custom styling wall)
  try {
    for (let i = 1; i <= 6; i++) {
      const pDoc = doc(db, 'instagram_posts', `post-${i}`);
      await deleteDoc(pDoc);
    }
  } catch (error) {
    console.warn('Non-blocking: Could not clear default mock instagram posts:', error);
  }
